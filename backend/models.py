...
class CandidateProfile(BaseModel):
    ...
    # v0.3.35 (Rule 11): security clearances / professional licenses /
    # certifications the candidate HOLDS, e.g. ["TS/SCI", "CPA"]. Set in the
    # setup wizard. None = NOT ANSWERED yet → the credential gate is OFF (no role
    # capped), so every existing profile is unaffected. [] = "I hold none" →
    # a JD that HARD-requires a clearance/license/cert the user lacks is capped
    # at MAYBE (often obtainable, so surfaced — not an apply-now match).
